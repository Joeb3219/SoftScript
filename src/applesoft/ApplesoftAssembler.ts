...
export class ApplesoftAssembler {
    ...
    private assembleLine(currentAddress: number, line: string): number[] {
        ...
        const operationOpcode = Object.entries(OpcodeToApplesoftInstructionMap).find(e => e[1] === operation)?.[0];

        if (!operationOpcode) {
            throw new Error(`Operation is invalid: ${operation}`);
        }

        const data = dataStr.join(' ').split('').map(c => c.charCodeAt(0));
    ...
    assemble(): number[] {
        const lineBytes = this.lines.reduce<number[]>((state, line) => {
            const currentAddress = state.length + ApplesoftAssembler._STARTING_ADDRESS;
