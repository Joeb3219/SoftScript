...
export class EvaluatorTarget {
    ...
    evaluateFunctionCall(
        ...
        const evaluatedArgs = node.arguments.map((arg) =>
            this.evaluateExpression(frame, arg.value)
        );
        ...
        switch (fnName) {
            ...
            default:
                ...
                if (evaluatedArgs.length !== fnEntry.parameters.length) {
                    throw new Error(
                        `Invoking function '${fnName}' with ${evaluatedArgs.length} args, but function requires ${fnEntry.parameters.length}`
                    );
                }
                ...
                return this.evaluateStatements(
                    frame,
                    fnEntry.functionDefinitionNode?.statements ?? []
                );
        }
    }

    evaluateExpressionNonMath(
        frame: StackFrame,
        ...
        switch (expression.value.type) {
            ...
            case "identifier":
                ...
                if (!variableState) {
                    throw new Error(
                        "Attempting to utilize variable before definition"
                    );
                }
    ...
    evaluateExpression(
        ...
        switch (expression.value.type) {
            ...
            case "expression_math": {
                const left = this.evaluateExpressionNonMath(
                    frame,
                ...
                if (typeof left !== "number" || typeof right !== "number") {
                    throw new Error(
                        `Attempting to evaluate math expression given types of ${typeof left} and ${typeof right}`
                    );
                }
        ...
        switch (statement.value.type) {
            ...
            case "variable_definition":
                frame.variables[statement.value.identifier.token.lexeme] = {
                    ...
                    value: statement.value.initialValue
                ...
                return frame.variables[statement.value.identifier.token.lexeme]
                    .value;
        }
    }
    ...
    evaluateStatements(
        ...
        for (const statement of statements) {
            ...
            if (
                statement.value.type === "expression" &&
                statement.value.value.type === "expression_return"
            ) {
                return value;
            }
        }
